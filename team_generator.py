"""Classes for generating two weighted randomly selected teams for Valorant scrims. 
"""
import enum
...
class TeamGenerator(object):
  '''Class for generating two weighted random Valorant teams.
  
  Players are randomly selected from a List[Player], which the rank of the
  Player increasing their odds of getting selected. 
  '''
  ...
  def generate(self, ignore_length: bool = False) -> str:
    if not ignore_length and len(self.players) != 10:
  ...
  def add_player(self, player_name: str, rank: Rank):
    if len(self.players) == 10:
      raise ValueError(f'Players list is full.')
    player = Player(player_name, rank)
    if player in self.players:
      raise ValueError(f'{player_name} is already added.')
